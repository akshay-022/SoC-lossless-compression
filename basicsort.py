...
# This is a generator function which can generate infinite-stream of inputs for traininghttp://shashank-gupta.com/2016/07/22/learning-to-sort-using-seq2seq/
...
def batch_gen(batch_size=32, seq_len=10, max_no=100):
	...
	# Generates a batch of input
        ...
        X = np.random.randint(low = i%90,high = i%90 +10 , size=(batch_size, seq_len))
...
from keras.models import Sequential
from keras.layers import recurrent, LSTM, TimeDistributed, Activation, RepeatVector, Dropout, Dense
...
model.add(LSTM(100, input_shape=(seq_len, max_no)))
...
for ind,(X,Y) in enumerate(batch_gen(batch_size, seq_len, max_no)):
    ...
    if ind % 250 == 0:
        ...
        if ind%500 == 0:
            testX = np.random.randint(low = (10*(ind//250) + 10)%90 ,high = (10*(ind//250) + 10)%90 +10, size=(1, seq_len))
        test = encode(testX, seq_len, max_no)
        print ('test number : ' + str(ind//250 +1))
