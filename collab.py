...
source,target = zip(*[[ar[i][k] for k in range(0,2)] for i in range(len(ar))])#this only works with tuples
...
def evaluate1(sentence):
    #attention_plot= np.zeros((max_source_length, max_target_length))
    #preprocess the sentnece
    inputs = [-10]+ sentence+ [-12]
    inputsi=inputs
...
for i in range(1):
    ...
    arr= a*np.sin(b*time)+c*np.sin(d*time)
    ar1=arr
    np.random.shuffle(arr)
    z=[[[-10]+arr.tolist()+[-12],[-10]+ar1.tolist()+[-12]]]#to convert numpy to a list do this its always more convenient i guess when you're not dealing wih super complex calculations
    ar=ar+z
    ...
    arr= a1*np.sin(b1*time)+c1*np.sin(d1*time)+e1*np.sin(f1*time)
    ar1=arr
    np.random.shuffle(arr)
    z=[[[-10]+arr.tolist()+[-12],[-10]+ar1.tolist()+[-12]]]#to convert numpy to a list do this its always more convenient i guess when you're not dealing wih super complex calculations
    ar=ar+z
...
# Amplitude of the sine wave is sine of a variable like time
# Plot a sine wave using time and amplitude obtained for the sine wave
plt.plot(time, ar[1][1][1:101])
plt.plot(time, translate(ar[1][0][1:101]))
plt.legend(["Dataset 1", "Dataset 2"])

# Give a title for the sine wave plot
plt.title('Sine wave superpsn')
# Give x axis label for the sine wave plot
plt.xlabel('Time')
# Give y axis label for the sine wave plot
plt.ylabel('wave')
